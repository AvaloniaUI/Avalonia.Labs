...
namespace Avalonia.Labs.Controls
{
    ...
    internal struct Velocity
    {
        ...
        public Vector PixelsPerSecond { get; }
        ...
        public Velocity ClampMagnitude(double minValue, double maxValue)
        {
            ...
            var valueSquared = PixelsPerSecond.SquaredLength;
            if (valueSquared > maxValue * maxValue)
            {
                var length = PixelsPerSecond.Length;
    ...
    internal struct VelocityEstimate(Vector PixelsPerSecond, double Confidence, TimeSpan Duration, Vector Offset)
    {
        public Vector PixelsPerSecond { get; } = PixelsPerSecond;
    ...
    internal class VelocityTracker
    {
        ...
        protected virtual VelocityEstimate? GetVelocityEstimate()
        {
            ...
            do
            {
                ...
                var age = (newestSample.Time - sample.Time).TotalMilliseconds;
                var delta = Math.Abs((sample.Time - previousSample.Time).TotalMilliseconds);
        ...
        internal Velocity GetVelocity()
        {
            ...
            if (estimate == null || estimate?.PixelsPerSecond == default)
            {
                return new Velocity(Vector.Zero);
            }
            return new Velocity(estimate?.PixelsPerSecond ?? default);
        }
    ...
    internal sealed class LeastSquaresSolver
    {
        ...
        public static PolynomialFit? Solve(int degree, ReadOnlySpan<double> x, ReadOnlySpan<double> y, ReadOnlySpan<double> w)
        {
            ...
            for (var j = 0; j < n; j += 1)
            {
                ...
                for (var i = 0; i < j; i += 1)
                {
                    var dot = Multiply(q.GetRow(j), q.GetRow(i));
                    for (var h = 0; h < m; h += 1)
                    {
